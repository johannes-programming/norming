.ONESHELL:
.PHONY: all amend beautiful black build clean commit isort jacobus pypi test toml_sorted_pyproject upload version works
...
rebase:
	git rebase --empty=drop --interactive -- "$(ROOT)";

test:
	@conda env remove -y -n test311 2>/dev/null || true;
	conda create -y -n test311 python=3.11;
	conda run -n test311 pip install -e .;
	conda run -n test311 python -c 'import norming.tests; norming.tests.test()';
	conda run -n test311 pip install mypy;
	conda run -n test311 python -m mypy -p norming;
